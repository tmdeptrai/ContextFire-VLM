...
STAGE_1_USER_QUERY = 'Please summarize what you see in this image. Respond only in this json format:{ "caption": "..." }'
...
def process_image_2_stage_transformers(model, processor, device, image_path):
    ...
    try:
        ...
        messages_s1 = [{"role": "system", "content": STAGE_1_SYSTEM_MESSAGE}, {"role": "user", "content": [{"type": "image"}, {"type": "text", "text": STAGE_1_USER_QUERY}]}]
        ...
        # --- Stage 2: Caption to Label ---
        messages_s2 = [{"role": "system", "content": STAGE_2_SYSTEM_MESSAGE}, {"role": "user", "content": STAGE_2_USER_QUERY.format(caption=intermediate_caption)}]
...
def parse_args():
    parser = argparse.ArgumentParser(description="2-Stage VLM Evaluation (Image->Caption->Label)")
    parser.add_argument("--model_name", type=str, required=True, help="Name for the model run")
    parser.add_argument("--input_csv", type=str, default="labels_v2.csv", help="Input CSV file")
    parser.add_argument("--output_dir", type=str, default="results_2_stage", help="Output directory")
