    ...
    try:
        ...
        messages = [{
            ...
            "content": [
                ...
                {"type": "text", "text": SYSTEM_MESSAGE + "" + USER_QUERY}
            ]
        }]
...
def process_image_lmdeploy(pipe, image_path):
    ...
    try:
        ...
        prompt = "<image>" + SYSTEM_MESSAGE + "" + USER_QUERY
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate a VLM on the fire detection dataset")
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        if os.path.exists(img_path):
            if args.backend == "openai":
                pred, caption, inf_time = process_image_openai(client, args.model_name, img_path)
            elif args.backend == "transformers":
                pred, caption, inf_time = process_image_transformers(model, processor, device, img_path)
            elif args.backend == "lmdeploy":
                pred, caption, inf_time = process_image_lmdeploy(pipe, img_path)
    ...
    plt.savefig(os.path.join(args.output_dir, f"{args.model_name}_inference_times.png"))
    plt.close()
    ...
    plt.ylabel('True')
    plt.tight_layout()
    plt.savefig(os.path.join(args.output_dir, f"{args.model_name}_confusion_matrix.png"))
    plt.close()
